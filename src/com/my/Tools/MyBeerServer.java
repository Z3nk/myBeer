...
public class MyBeerServer {
	...
	public static final String BEER_ADRESSE = "http://172.19.129.82:3000";
		...
		while ((output = br.readLine()) != null) {
			if (output.contains("_id"))
				return output.substring(output.indexOf(':') + 1)
						.replaceAll("\"", "").trim();
			// En theorie "_id": "549454d59ab258241958c25a" devient :
			// 549454d59ab258241958c25a
	...
	public static String addBeer(Beer b) throws Exception{
		URL url = new URL(BEER_ADRESSE + "/beers");
		HttpURLConnection conn = (HttpURLConnection) url.openConnection();
		conn.setDoOutput(true);
		conn.setRequestMethod("POST");
		conn.setRequestProperty("Content-Type", "application/json");
		...
		String output;
		System.out.println("[MyBeerServer][addBeer][141].... \n");
		while ((output = br.readLine()) != null) {
			if (output.contains("_id"))
				return output.substring(output.indexOf(':') + 1)
						.replaceAll("\"", "").trim();
