...
public class MainActivity extends FragmentActivity implements LocationListener {
		...
		if (googleMap == null) {
			...
			googleMap.setOnMarkerClickListener(new OnMarkerClickListener() {
				...
				public boolean onMarkerClick(Marker marker) {
					...
					/*
					 * List<Bar> l = DAO.getBars(); for(Bar bartmp : l){
					 * if(bartmp.getPos().equals(arg0.getPosition())){ Intent
					 * intention = new
					 * Intent(MainActivity.this.getApplicationContext(),
					 * BarActivity.class);
					 * System.out.println("[MainActivity][onMarkerClick][96]"
					 * +bartmp.toString()); intention.putExtra("id",
					 * bartmp.getId()); intention.putExtra("pos",
					 * bartmp.getPos()); startActivity(intention); } }
					 * Toast.makeText(getApplicationContext(), "YOLO",
							...
							for (Bar bartmp : l) {
								if (bartmp.getPos().equals(arg0.getPosition())) {
									Intent intention = new Intent(
											...
											BarActivity.class);
									System.out
											.println("[MainActivity][onMarkerClick][96]"
													+ bartmp.toString());
			...
			googleMap.setOnMapLongClickListener(new OnMapLongClickListener() {
				...
				public void onMapLongClick(LatLng arg0) {
					Intent intention = new Intent(MainActivity.this
							.getApplicationContext(), AddBarActivity.class);
	...
	protected void onResume() {
		...
		/*
		 * if(Bars!=null) for(Bar b:Bars){ MarkerOptions M=new MarkerOptions();
		 * M.position(b.getPos()); M.title(b.getName());
		 * M.snippet(GoogleMapTools.getNbBeers(b.getBeers())+" bieres");
		 * M.icon(BitmapDescriptorFactory.fromPath(
	...
	public void onLocationChanged(Location location) {
		...
		textView.setText("Latitude:" + latLng.latitude + ", Longitude:"
				+ latLng.longitude);

		// A mettre dans le onresume et recuperer une derniere position recente
		// en variable de classe !
		if (isJustSynchroniseNeedToAddMarker
				|| lastPosition == null
				|| 15 < GoogleMapTools.DistanceBetweenPlaces(latLng.longitude,
						latLng.latitude, lastPosition.longitude,
