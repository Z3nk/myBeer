...
public class AddBarAutoCompleteAdressThread extends Thread {
	...
	private Place p;
    public AddBarAutoCompleteAdressThread(Bar b,EditText name, EditText adresse, AddBarActivity activity) {
    	this.name=name;
	...
	public void run(){
		p = new Place();
		ArrayList<Place> list=PlacesService.search(b.getPos().latitude, b.getPos().longitude, 5);
		if(list.size()>0){
