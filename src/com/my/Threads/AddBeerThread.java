...
public class AddBeerThread extends Thread {
	...
	public void run() {
		...
		try {
			...
			// System.out.println("[AddBeerThread][Run][18] On a ajoutÃ© un bar avec un idserveur :"+
			// _idInServerBase);
			b.setIdServer(_idInServerBase);
		} catch (Exception e) {
			// System.out.println("[AddBeerThread][Run][22] Erreur base de donnÃ©e externe : "+
			// e.toString());
		}
		if (b.getIdServer() != null) {
			...
			activity.runOnUiThread(new Runnable() {
			    public void run() {
			        Toast.makeText(activity, "Impossible de rajouter une bière, nous n'avons pas accès au serveur ! :(", Toast.LENGTH_SHORT).show();
			    }
			});
		}
